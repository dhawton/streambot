...
client.on("presenceUpdate", (_, newPresence): void => {
    ...
    if (activity.type === "STREAMING" && !Streamers[newPresence.user.tag] && Utils.isVBRP(activity.details)) {
      ...
      twitch
        ...
        .then((streamData) => {
          ...
          guild.members.cache.find((member) => member.user.tag === newPresence.user.tag).roles.add(streamRole);
        })
        .catch(() => {
...
setInterval(() => {
  if (!streamLock) {
    ...
    Streams.forEach((v) => {
      twitch.fetchStreamInfo(v.channel).then((streamData) => {
        if (!streamData) {
          newStreams.filter((s) => s.channel === v.channel);
          guild.members.cache.find((member) => member.user.tag === v.tag).roles.remove(streamRole);
        }
      });
    });
    Streams = newStreams;
